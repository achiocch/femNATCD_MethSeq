...
boxplot_counts = function(plotsubset, maintitle, colorcode) {
  ...
  legend(length(a$n)*1.05, max(dat), legend = levels(Patdata[,colorcode]),
         pch = 16, col = Dark8[1:length(unique(Patdata[,colorcode]))],
         xpd=T,bty="n")
}
...
barplot_counts = function(DF, maintitle, colorcode) {
  ...
  par(mar=c(5,5,5,8))
  a = barplot(dat, main = maintitle, border=NA,
          col = Dark8[as.factor(DF[,colorcode])], names="",
          ylab = "log2 transformed",xlab="samples")
  legend(max(a)*1.05, max(dat), xpd=T,
         legend = levels(DF[,colorcode]), bty="n",
         pch = 16, col = Dark8[1:length(unique(DF[,colorcode]))])
}
